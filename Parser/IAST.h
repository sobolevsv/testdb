...
public:
    ...
    {
        throw Exception("Trying to get name of not a column: " + getID());
    }
    ...
    {
        throw Exception("Can't set alias of " + getColumnName());
    }
...
//    virtual void updateTreeHashImpl(SipHash & hash_state) const;
    ...
    void set(T * & field, const ASTPtr & child)
    {
        ...
        if (!casted)
            throw Exception("Could not cast AST subtree");
    ...
    {
        if (!child)
            throw Exception("Trying to replace AST subtree with nullptr");
        ...
        throw Exception("AST subtree not found in children");
    }
