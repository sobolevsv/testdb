...
String getIdentifierName(const IAST * ast)
{
    ...
    if (tryGetIdentifierNameInto(ast, res))
        return res;
    //throw Exception(ast ? queryToString(*ast) + " is not an identifier" : "AST node is nullptr");
    throw Exception(ast ? "queryToString(*ast)  is not an identifier" : "AST node is nullptr");
}
...
//{
//    if (!ast)
//        throw Exception("AST node is nullptr");
//    const auto & identifier = dynamic_cast<const ASTIdentifier &>(*ast);
//    if (identifier.name_parts.size() > 2)
//        throw Exception("Logical error: more than two components in table expression");
//
//    if (identifier.name_parts.size() == 2)
