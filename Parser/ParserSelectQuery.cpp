...
//                return false;
        ...
        if (s_by.ignore(pos, expected))
        {
            ...
            if (limit_with_ties_occured)
                throw Exception("Can not use WITH TIES alongside LIMIT BY");
        ...
        if (top_length && limit_length)
            throw Exception("Can not use TOP and LIMIT together");
    }
    else if (s_offset.ignore(pos, expected))
    {
//        if (!exp_elem.parse(pos, limit_offset, expected))
    ...
    if (!order_expression_list && select_query->limit_with_ties)
        throw Exception("Can not use WITH TIES without ORDER BY");
